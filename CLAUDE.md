...
- BetterAuth `auth.api.getFullOrganization()` throws (does NOT return `null`) when the organization slug doesn't exist. Always wrap in try-catch and treat the caught error as a 404. <!-- learned 2026-02-24 -->
- BetterAuth `getFullOrganization` is session-scoped — it only returns orgs the requesting user is a member of. Non-members get null/throw, so the HOF returns 404 (not 403). This is correct: don't leak org existence to outsiders. <!-- learned 2026-02-24 -->
- BetterAuth `admin()`, `apiKey()`, `bearer()` plugins are exported from `better-auth/plugins`. `apiKeyClient()` is from `better-auth/client/plugins`. `createAuthMiddleware` is from `better-auth/api`. <!-- learned 2026-02-24 -->
- `import { after } from "next/server"` is stable since Next.js 15.1.0. Use it for async side-effects (logging, analytics) that should not block the response. <!-- learned 2026-02-24 -->
- Zod validation in HOFs: pass `bodySchema` option to the HOF, call `schema.parse()` inside the HOF, and let `handleApiError` catch `z.ZodError` → 422 with field-level messages. <!-- learned 2026-02-24 -->
